...
// Supports: # / ## / ### headings, - or * bullet lists, 1. numbered lists,
// **bold**, *italic* / _italic_, `inline code`, and paragraphs.
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {blocks.map((bl, i) => {
        if (bl.t === 'h') {
          ...
          return <div key={i} style={{ fontSize: size, fontWeight: 700, color: 'var(--fg)', marginTop: i ? 4 : 0 }}>{inline(bl.text, i)}</div>;
        }
        if (bl.t === 'ul' || bl.t === 'ol') {
          return (
            <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {bl.items.map((it, j) => (
                <div key={j} style={{ display: 'flex', gap: 8, fontSize: 12.5, lineHeight: 1.55 }}>
        ...
        return <div key={i} style={{ fontSize: 12.5, lineHeight: 1.6 }}>{inline(bl.text, i)}</div>;
      })}
    </div>
  );
}
...
export default function VitalsChat(props) {
  ...
  const deptOf = (name) => {
    if (!rND) return '';
    const m = /\(([^)]+)\)\s*$/.exec(String(rND(name)));
    return m ? m[1] : '';
  };
  ...
  const standings = useMemo(() => {
    ...
    const rows = Object.entries(perfMetrics.people)
      ...
      .map(([name, d]) => ({
        name, dept: deptOf(name),
        completed: d.completed || 0, onTime: d.onTime || 0,
        overdue: d.overdue || 0, onTimeRate: d.onTimeRate || 0,
      }))
  ...
  function buildContext() {
    ...
    const overdue = tasks.filter(t => t.status !== 'Done' && dstr(t.end_date) && dstr(t.end_date) < today)
      .slice(0, 25).map(t => ({ task: t.name, owner: t.owner, dept: t.dept, due: dstr(t.end_date) }));
    const in7 = new Date(Date.now() + 7 * 86400000).toISOString().split('T')[0];
    const dueSoon = tasks.filter(t => t.status !== 'Done' && dstr(t.end_date) && dstr(t.end_date) >= today && dstr(t.end_date) <= in7)
      .slice(0, 25).map(t => ({ task: t.name, owner: t.owner, dept: t.dept, due: dstr(t.end_date) }));
    const activeRisks = risks.filter(r => (r.status || '').toUpperCase() !== 'CLOSED')
      .slice(0, 25).map(r => ({ risk: r.description, impact: r.impact, status: r.status, owner: r.owner }));
    const offNow = leaves.filter(l => l.status === 'approved' && dstr(l.start_date) <= today && dstr(l.end_date) >= today)
      .map(l => ({ person: l.person, type: l.leave_type, until: dstr(l.end_date) }));
    const upcoming = leaves.filter(l => l.status === 'approved' && dstr(l.start_date) > today)
      .slice(0, 25).map(l => ({ person: l.person, type: l.leave_type, from: dstr(l.start_date), to: dstr(l.end_date) }));
    const openDecisions = decisions.filter(d => (d.status || 'open') === 'open')
      .slice(0, 20).map(d => ({ title: d.title, owner: d.owner, priority: d.priority, due: dstr(d.due_date) }));
    const flaggedKpis = kpis.filter(k => k.flag && k.flag !== 'green')
      .slice(0, 20).map(k => ({ kpi: k.name, dept: k.dept, current: k.current_value, target: k.target, flag: k.flag }));
    const openRoles = roles.filter(r => r.status && r.status !== 'Filled')
      .slice(0, 20).map(r => ({ role: r.title, status: r.status }));
    ...
    const performance = perfMetrics && perfMetrics.people
      ? Object.entries(perfMetrics.people).filter(([n]) => n !== 'Efehan Maleri')
          .map(([name, d]) => ({ name, dept: deptOf(name), assigned: d.assigned, completed: d.completed, onTime: d.onTime, overdue: d.overdue, onTimeRate: d.onTimeRate }))
      : 'not loaded';
    return {
      note: 'Efehan (CEO) is excluded from performance and leave. Performance figures are current standings across tracked Linear/Asana work (not date-windowed yet), so treat them as cumulative, not "this week".',
      today,
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 1000, margin: '0 auto', width: '100%' }}>
      <style>{`@keyframes vcblink{0%,80%,100%{opacity:.25}40%{opacity:1}} .vc-dot{width:6px;height:6px;border-radius:50%;background:var(--fg2);display:inline-block;animation:vcblink 1.2s infinite both}`}</style>
      ...
      <div style={{ ...card, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)' }}>
          ...
          <div style={{ fontSize: 10, color: fg2, marginTop: 2 }}>Answers from your live platform data. It does not make up names or numbers; if the data does not cover something, it says so.</div>
        </div>

        <div ref={scrollRef} style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 18, minHeight: 240, maxHeight: 520, overflowY: 'auto' }}>
          {messages.length === 0 && (
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {suggestions.map(s => (
                  <button key={s} onClick={() => send(s)} disabled={loading}
                    style={{ background: 'var(--bg2)', border: '1px solid var(--border)', color: fg, borderRadius: 999, padding: '7px 13px', fontSize: 11.5, cursor: loading ? 'default' : 'pointer' }}>
                    {s}
                  </button>
                ))}
              </div>
              ...
              <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                ...
                <div style={{ paddingLeft: 25 }}>
                  {m.warn
                    ...
                    : <div style={{ color: fg }}><Markdown text={m.text} /></div>}
                </div>
              </div>
            )
          ))}
          ...
          {loading && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                <div style={{ width: 18, height: 18, borderRadius: 5, background: 'var(--bg3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 800, color: fg2 }}>A</div>
                <div style={{ fontSize: 10.5, fontWeight: 600, color: fg2 }}>Ops Assistant</div>
              </div>
              <div style={{ paddingLeft: 25, display: 'flex', gap: 4, alignItems: 'center', height: 18 }}>
                <span className="vc-dot" /><span className="vc-dot" style={{ animationDelay: '.2s' }} /><span className="vc-dot" style={{ animationDelay: '.4s' }} />
              </div>
            </div>
          )}
        </div>
        ...
        <div style={{ padding: 12, borderTop: '1px solid var(--border)', display: 'flex', gap: 8 }}>
          ...
          <button onClick={() => send()} disabled={loading || !input.trim()}
            style={{ background: 'var(--fg)', color: 'var(--bg)', border: 'none', borderRadius: 8, padding: '0 20px', fontSize: 12, fontWeight: 700, cursor: loading || !input.trim() ? 'default' : 'pointer', opacity: loading || !input.trim() ? 0.5 : 1 }}>
            Send
          </button>
        </div>
      </div>
