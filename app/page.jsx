...
// Health color for hover
function healthColor(task){
  if(!task)return"transparent";
  if(task.status==="Done")return"#DCFCE7";
  if(task.risk==="Off track"||task.status==="To Do"&&task.end_date<today)return"#FEE2E2";
  if(task.risk==="At risk")return"#FFF7ED";
  return"#F0FFF4";
}
...
.ani-fade{animation:fadeIn .3s ease-out both}
.ani-slide{animation:slideIn .3s ease-out both}
.ani-scale{animation:scaleIn .25s ease-out both}
.card-hover{transition:all .2s ease}
.card-hover:hover{transform:translateY(-2px);box-shadow:0 4px 12px rgba(0,0,0,.1)!important}
.row-hover{transition:all .15s ease}
.row-hover:hover{background:var(--hover)!important}
.bar-ani{animation:growBar .6s ease-out both}
.tab-btn{transition:all .15s ease}
.tab-btn:hover{color:var(--fg)!important}
/* Dark mode */
[data-theme="dark"]{--bg:#0F172A;--bg2:#1E293B;--bg3:#334155;--fg:#F1F5F9;--fg2:#94A3B8;--border:#334155;--hover:rgba(59,130,246,.1);--card:#1E293B;--header:linear-gradient(135deg,#0F172A,#1E293B)}
[data-theme="light"]{--bg:#FFFFFF;--bg2:#F8FAFC;--bg3:#F1F5F9;--fg:#1E293B;--fg2:#64748B;--border:#E8ECEF;--hover:#F8FAFC;--card:#FFFFFF;--header:linear-gradient(135deg,#0D1B2A,#1B3A5C)}
body{background:var(--bg);color:var(--fg);transition:background .3s,color .3s}
`;

function InEdit({value,onChange,type="text",options,dark}){
  ...
  const sc={fontSize:12,color:"var(--fg)",background:"transparent"};
  if(type==="select")return <select value={val} onChange={e=>{setVal(e.target.value);onChange(e.target.value)}} style={{...sc,border:"none",cursor:"pointer",padding:"2px 4px",borderRadius:4}}>{options.map(o=><option key={o}>{o}</option>)}</select>;
  if(type==="date")return <input type="date" value={val} onChange={e=>{setVal(e.target.value);onChange(e.target.value)}} style={{...sc,border:"1px solid var(--border)",borderRadius:4,padding:"2px 4px",width:120}}/>;
  if(!editing)return <span onClick={()=>setEditing(true)} style={{...sc,cursor:"text",padding:"2px 4px",borderRadius:4,minWidth:40,display:"inline-block"}}>{val||"—"}</span>;
  return <input ref={ref} value={val} onChange={e=>setVal(e.target.value)} onBlur={()=>{setEditing(false);onChange(val)}} onKeyDown={e=>{if(e.key==="Enter"){setEditing(false);onChange(val)}}} style={{...sc,border:"1px solid #3B82F6",borderRadius:4,padding:"2px 4px",width:140}}/>;
}

function AddModal({title,fields,onSave,onClose}){
  const[vals,setVals]=useState({});
  return <div className="ani-fade" style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(4px)"}} onClick={onClose}>
    <div className="ani-scale" onClick={e=>e.stopPropagation()} style={{background:"var(--card)",borderRadius:16,width:440,padding:24,boxShadow:"0 25px 60px rgba(0,0,0,.3)",border:"1px solid var(--border)"}}>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:16}}><h3 style={{margin:0,fontSize:16,fontWeight:800,color:"var(--fg)"}}>{title}</h3><button onClick={onClose} style={{background:"none",border:"none",fontSize:18,cursor:"pointer",color:"var(--fg2)"}}>x</button></div>
      ...
      <button onClick={()=>onSave(vals)} style={{width:"100%",padding:10,background:"linear-gradient(135deg,#3B82F6,#6366F1)",color:"#fff",border:"none",borderRadius:8,fontWeight:700,fontSize:13,cursor:"pointer",marginTop:8,transition:"transform .15s"}} onMouseEnter={e=>e.target.style.transform="scale(1.02)"} onMouseLeave={e=>e.target.style.transform="scale(1)"}>Save</button>
    </div>
  </div>;
}
...
function TicketPopup({task,tasks,onClose,onUpdate,onDelete}){
  ...
  return <div className="ani-fade" style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(4px)"}} onClick={onClose}>
    <div className="ani-scale" onClick={e=>e.stopPropagation()} style={{background:"var(--card)",borderRadius:16,width:500,maxHeight:"85vh",overflow:"auto",boxShadow:"0 25px 60px rgba(0,0,0,.3)",border:"1px solid var(--border)"}}>
      <div style={{borderBottom:"4px solid "+cl,padding:"20px 24px 16px",background:overdue?"#FEE2E2":"transparent",borderRadius:"16px 16px 0 0"}}>
        <div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontSize:10,color:"var(--fg2)",fontWeight:600}}>TASK-{task.id} / {task.dept}{overdue?" — OVERDUE":""}</span><button onClick={onClose} style={{background:"none",border:"none",fontSize:18,cursor:"pointer",color:"var(--fg2)"}}>x</button></div>
        <div style={{fontSize:18,fontWeight:800,marginTop:4,color:"var(--fg)"}}><InEdit value={task.name} onChange={v=>onUpdate(task.id,{name:v})}/></div>
        <div style={{display:"flex",gap:6,marginTop:8,flexWrap:"wrap"}}><Bdg bg={cl+"20"} c={cl}>{task.dept}</Bdg>{pc&&<Bdg bg={pc.bg} c={pc.c}>{task.priority}</Bdg>}{rc&&<Bdg bg={rc.bg} c={rc.c}>{task.risk}</Bdg>}{overdue&&<Bdg bg="#DC2626" c="#fff">OVERDUE</Bdg>}</div>
      </div>
      <div style={{padding:"16px 24px",display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,fontSize:13,color:"var(--fg2)"}}>
        <div><div style={{fontSize:10,color:"var(--fg2)",fontWeight:600}}>Owner</div><InEdit value={task.owner} onChange={v=>onUpdate(task.id,{owner:v})}/></div>
        <div><div style={{fontSize:10,color:"var(--fg2)",fontWeight:600}}>Department</div><InEdit value={task.dept} onChange={v=>onUpdate(task.id,{dept:v})} type="select" options={DEPT_OPT}/></div>
        <div><div style={{fontSize:10,color:"var(--fg2)",fontWeight:600}}>Start</div><InEdit value={task.start_date} onChange={v=>onUpdate(task.id,{start_date:v})} type="date"/></div>
        <div><div style={{fontSize:10,color:"var(--fg2)",fontWeight:600}}>End</div><InEdit value={task.end_date} onChange={v=>onUpdate(task.id,{end_date:v})} type="date"/></div>
        <div><div style={{fontSize:10,color:"var(--fg2)",fontWeight:600}}>Priority</div><InEdit value={task.priority} onChange={v=>onUpdate(task.id,{priority:v})} type="select" options={PRI_OPT}/></div>
        <div><div style={{fontSize:10,color:"var(--fg2)",fontWeight:600}}>Duration</div><b>{dB(task.start_date,task.end_date)||1}d</b></div>
      </div>
      <div style={{padding:"0 24px 16px"}}><div style={{fontSize:10,color:"var(--fg2)",fontWeight:600,marginBottom:4}}>Status</div><div style={{display:"flex",gap:6}}>{STS.map(s=><button key={s} onClick={()=>onUpdate(task.id,{status:s})} style={{padding:"6px 14px",borderRadius:8,border:task.status===s?"2px solid var(--fg)":"1px solid var(--border)",background:task.status===s?"var(--fg)":"var(--card)",color:task.status===s?"var(--bg)":"var(--fg2)",fontSize:12,fontWeight:600,cursor:"pointer",transition:"all .15s"}}>{s}</button>)}</div></div>
      <div style={{padding:"0 24px 16px"}}><div style={{fontSize:10,color:"var(--fg2)",fontWeight:600,marginBottom:4}}>Risk</div><div style={{display:"flex",gap:6}}>{RSK_OPT.map(r=>{const rc2=RC[r];return <button key={r} onClick={()=>onUpdate(task.id,{risk:r})} style={{padding:"4px 12px",borderRadius:99,border:task.risk===r?"2px solid "+rc2.c:"1px solid var(--border)",background:rc2.bg,color:rc2.c,fontSize:11,fontWeight:700,cursor:"pointer",transition:"all .15s"}}>{r}</button>})}</div></div>
      {depN.length>0&&<div style={{padding:"0 24px 12px"}}><div style={{fontSize:10,color:"var(--fg2)",fontWeight:600}}>Depends On</div>{depN.map((n,i)=><div key={i} style={{fontSize:12,color:"#6366F1"}}>→ {n}</div>)}</div>}
      {blocks.length>0&&<div style={{padding:"0 24px 12px"}}><div style={{fontSize:10,color:"var(--fg2)",fontWeight:600}}>Blocks</div>{blocks.map((n,i)=><div key={i} style={{fontSize:12,color:"#DC2626"}}>← {n}</div>)}</div>}
      <div style={{padding:"0 24px 20px"}}><button onClick={()=>{onDelete(task.id);onClose()}} style={{background:"#FEE2E2",color:"#DC2626",border:"none",padding:"6px 14px",borderRadius:8,fontSize:11,fontWeight:700,cursor:"pointer",transition:"all .15s"}} onMouseEnter={e=>e.target.style.background="#FECACA"} onMouseLeave={e=>e.target.style.background="#FEE2E2"}>Delete Task</button></div>
    </div>
  </div>;
}
...
function KpiChart({kpis}){
  ...
  return <div className="ani-fade" style={{overflowX:"auto",marginBottom:16,background:"var(--bg2)",borderRadius:12,padding:16,border:"1px solid var(--border)"}}>
    ...
    <svg width={w} height={h} style={{fontFamily:"Inter,system-ui"}}>{depts.map((d,i)=>{
      ...
      return <g key={d}>
        <rect x={x} y={bH-gH} width={bW} height={gH} fill="#16A34A" rx={4} opacity={0.85}><animate attributeName="height" from="0" to={gH} dur="0.6s" fill="freeze"/><animate attributeName="y" from={bH} to={bH-gH} dur="0.6s" fill="freeze"/></rect>
        <rect x={x} y={bH-gH-yH} width={bW} height={yH} fill="#F59E0B" rx={0}><animate attributeName="height" from="0" to={yH} dur="0.6s" fill="freeze" begin="0.1s"/><animate attributeName="y" from={bH-gH} to={bH-gH-yH} dur="0.6s" fill="freeze" begin="0.1s"/></rect>
        <rect x={x} y={bH-gH-yH-rH} width={bW} height={rH} fill="#EF4444" rx={4}><animate attributeName="height" from="0" to={rH} dur="0.6s" fill="freeze" begin="0.2s"/><animate attributeName="y" from={bH-gH-yH} to={bH-gH-yH-rH} dur="0.6s" fill="freeze" begin="0.2s"/></rect>
        <rect x={x} y={bH+4} width={bW} height={5} fill={CL[d]||"#94A3B8"} rx={2.5}/>
        <text x={x+bW/2} y={bH+28} textAnchor="middle" fill="var(--fg)" fontSize={10} fontWeight={600}>{d.length>9?d.slice(0,8)+"..":d}</text>
        <text x={x+bW/2} y={bH+44} textAnchor="middle" fill="var(--fg2)" fontSize={9}>{data.green} on track</text>
        <text x={x+bW/2} y={bH+56} textAnchor="middle" fill="var(--fg2)" fontSize={9}>{data.yellow} attention · {data.red} under</text>
      </g>})}</svg>
  </div>;
}

function CalendarView({tasks,meetings,onSelect}){
  ...
  return <div className="ani-fade">
    <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16}}>
      <button onClick={()=>setMonth(new Date(yr,mo-1,1))} style={{background:"var(--bg3)",border:"none",borderRadius:8,padding:"8px 14px",cursor:"pointer",fontWeight:700,color:"var(--fg)",transition:"all .15s"}}>←</button>
      <span style={{fontSize:16,fontWeight:700,color:"var(--fg)"}}>{month.toLocaleDateString("en-GB",{month:"long",year:"numeric"})}</span>
      <button onClick={()=>setMonth(new Date(yr,mo+1,1))} style={{background:"var(--bg3)",border:"none",borderRadius:8,padding:"8px 14px",cursor:"pointer",fontWeight:700,color:"var(--fg)",transition:"all .15s"}}>→</button>
      <a href="https://calendar.google.com" target="_blank" rel="noopener" style={{marginLeft:"auto",fontSize:11,color:"#3B82F6",fontWeight:600,textDecoration:"none",background:"#EFF6FF",padding:"8px 14px",borderRadius:8,transition:"all .15s"}}>+ Connect Google Calendar</a>
    </div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:1,background:"var(--border)",borderRadius:12,overflow:"hidden"}}>
      ...
      {cells.map((day,i)=>{if(!day)return <div key={i} style={{background:"var(--bg2)",minHeight:90}}/>;
        ...
        return <div key={i} className="card-hover" style={{background:"var(--card)",minHeight:90,padding:4,cursor:"default"}}>
          ...
          {dt.slice(0,3).map(t=><div key={t.id} onClick={()=>onSelect(t)} style={{background:CL[t.dept]||"#94A3B8",color:"#fff",fontSize:9,fontWeight:600,padding:"2px 4px",borderRadius:4,marginBottom:1,cursor:"pointer",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",opacity:t.status==="Done"?.35:.85,transition:"all .15s"}}>{t.name}</div>)}
          {dt.length>3&&<div style={{fontSize:9,color:"var(--fg2)"}}>+{dt.length-3} more</div>}
        </div>})}
    </div>
  </div>;
}

function Tbl({headers,rows}){return <div style={{border:"1px solid var(--border)",borderRadius:10,overflow:"hidden"}} className="ani-fade"><table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}><thead><tr style={{background:"var(--bg3)"}}>{headers.map(h=><th key={h} style={{padding:"10px 8px",textAlign:"left",fontWeight:600,color:"var(--fg2)",fontSize:11}}>{h}</th>)}</tr></thead><tbody>{rows.map((row,ri)=><tr key={ri} className="row-hover" style={{borderBottom:"1px solid var(--border)"}}>{row.map((cell,ci)=><td key={ci} style={{padding:"8px",color:"var(--fg)"}}>{cell}</td>)}</tr>)}</tbody></table></div>}
function DeptHdr({dept}){return <div style={{background:(CL[dept]||"#94A3B8")+"15",color:CL[dept],padding:"8px 12px",borderRadius:"8px 8px 0 0",fontWeight:700,fontSize:13,borderLeft:"4px solid "+(CL[dept]||"#94A3B8")}}>{dept}</div>}
...
export default function Home(){
  ...
  useEffect(()=>{async function la(){const[t,r,ri,k,m,ro]=await Promise.all([supabase.from('tasks').select('*').order('id'),supabase.from('raci').select('*').order('dept,id'),supabase.from('risks').select('*').order('id'),supabase.from('kpis').select('*').order('dept,id'),supabase.from('meetings').select('*').order('id'),supabase.from('roles').select('*').order('id')]);
    ...
    const ch=supabase.channel('rt2').on('postgres_changes',{event:'*',schema:'public',table:'tasks'},()=>supabase.from('tasks').select('*').order('id').then(({data})=>{if(data)setTasks(data)})).on('postgres_changes',{event:'*',schema:'public',table:'risks'},()=>supabase.from('risks').select('*').order('id').then(({data})=>{if(data)setRisks(data)})).on('postgres_changes',{event:'*',schema:'public',table:'kpis'},()=>supabase.from('kpis').select('*').order('dept,id').then(({data})=>{if(data)setKpis(data)})).on('postgres_changes',{event:'*',schema:'public',table:'raci'},()=>supabase.from('raci').select('*').order('dept,id').then(({data})=>{if(data)setRaci(data)})).on('postgres_changes',{event:'*',schema:'public',table:'roles'},()=>supabase.from('roles').select('*').order('id').then(({data})=>{if(data)setRoles(data)})).on('postgres_changes',{event:'*',schema:'public',table:'meetings'},()=>supabase.from('meetings').select('*').order('id').then(({data})=>{if(data)setMeetings(data)})).subscribe();
  ...
  // Drag and drop
  const onDrop=useCallback((newStatus)=>{if(dragId){updateTask(dragId,{status:newStatus});setDragId(null)}},[dragId,updateTask]);
  ...
  const stats=useMemo(()=>({total:tasks.length,todo:tasks.filter(t=>t.status==="To Do").length,doing:tasks.filter(t=>t.status==="Doing").length,done:tasks.filter(t=>t.status==="Done").length,risk:tasks.filter(t=>t.risk!=="On track").length,overdue:tasks.filter(t=>t.status!=="Done"&&t.end_date<today).length}),[tasks]);
  ...
  if(loading)return <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",fontSize:16,color:"var(--fg2)",fontFamily:"Inter,system-ui",background:"var(--bg)"}}>Loading Attimo...</div>;
  ...
  return <div style={{fontFamily:"'Inter',system-ui",background:"var(--bg)",minHeight:"100vh",transition:"all .3s"}}>
    ...
    {/* Header */}
    <div style={{background:"var(--header)",padding:"12px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        <div style={{width:28,height:28,borderRadius:8,background:"linear-gradient(135deg,#3B82F6,#6366F1)",display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{color:"#fff",fontSize:14,fontWeight:800}}>A</span></div>
        <span style={{fontSize:16,fontWeight:800,color:"#fff"}}>Attimo</span><span style={{color:"#475569"}}>|</span><span style={{fontSize:12,color:"#94A3B8"}}>Company Operations</span>
      </div>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        <button onClick={()=>setDark(!dark)} style={{background:"rgba(255,255,255,.1)",border:"none",borderRadius:8,padding:"6px 12px",cursor:"pointer",fontSize:11,color:"#fff",fontWeight:600,transition:"all .2s"}} title="Toggle dark/light mode">{dark?"☀ Light":"◑ Dark"}</button>
        <button onClick={doSync} disabled={syncing} style={{background:syncing?"rgba(255,255,255,.1)":"rgba(59,130,246,.8)",color:"#fff",border:"none",padding:"6px 14px",borderRadius:8,fontWeight:600,fontSize:11,cursor:syncing?"wait":"pointer",transition:"all .2s"}}>{syncing?"Syncing...":"Sync All"}</button>
        ...
        <div style={{display:"flex",gap:8,fontSize:11,color:"#94A3B8"}}>
          ...
          {stats.overdue>0&&<span><b style={{color:"#FCA5A5"}}>{stats.overdue}</b> overdue</span>}
        </div>
      </div>
    </div>
    ...
    <div style={{borderBottom:"1px solid var(--border)",padding:"0 20px",display:"flex",flexWrap:"wrap",background:"var(--card)"}}>
      {TABS.map(t=><button key={t.id} className="tab-btn" onClick={()=>setView(t.id)} style={{padding:"10px 14px",border:"none",background:"none",fontWeight:600,fontSize:13,cursor:"pointer",color:view===t.id?"var(--fg)":"var(--fg2)",borderBottom:view===t.id?"2px solid #3B82F6":"2px solid transparent"}}>{t.l}</button>)}
    ...
    {/* TIMELINE */}
    {view==="timeline"&&<div className="ani-fade">
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}><div style={{fontSize:14,fontWeight:800,color:"var(--fg)"}}>Company Gantt</div><button onClick={()=>setAddModal("task")} style={{background:"linear-gradient(135deg,#3B82F6,#6366F1)",color:"#fff",border:"none",padding:"6px 14px",borderRadius:8,fontWeight:600,fontSize:11,cursor:"pointer"}}>+ Add Task</button></div>
      <div style={{background:"var(--card)",borderRadius:10,border:"1px solid var(--border)"}}>{STS.map(st=>{const items=tasks.filter(t=>t.status===st);return <div key={st} style={{padding:"12px 16px",borderBottom:"1px solid var(--border)"}}>
        ...
        {items.map((t,idx)=>{const cl=CL[t.dept]||"#94A3B8";const sOff=Math.max(0,dB("2026-04-25",t.start_date));const dur=Math.max(1,dB(t.start_date,t.end_date)+1);const totalD=dB("2026-04-25","2026-07-05");const overdue=t.status!=="Done"&&t.end_date<today;
          return <div key={t.id} className="row-hover ani-slide" style={{display:"flex",alignItems:"center",gap:12,padding:"4px 0",cursor:"pointer",borderRadius:6,animationDelay:idx*30+"ms",borderLeft:overdue?"3px solid #EF4444":"3px solid transparent",paddingLeft:8}} onClick={()=>setSel(t)}>
            <div style={{width:200,display:"flex",alignItems:"center",gap:6,flexShrink:0}}><div style={{width:18,height:18,borderRadius:"50%",background:cl,display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{color:"#fff",fontSize:8,fontWeight:700}}>{t.owner?.[0]}</span></div><span style={{fontSize:12,fontWeight:500,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",color:"var(--fg)"}}>{t.name}</span></div>
            <div style={{flex:1,height:24,background:"var(--bg3)",borderRadius:6,position:"relative",overflow:"hidden"}}><div className="bar-ani" style={{position:"absolute",left:(sOff/totalD)*100+"%",width:Math.max((dur/totalD)*100,.8)+"%",top:2,bottom:2,borderRadius:4,background:overdue?"#EF4444":cl,opacity:t.status==="Done"?.3:.85,display:"flex",alignItems:"center",paddingLeft:6}}><span style={{color:"#fff",fontSize:9,fontWeight:600,whiteSpace:"nowrap",overflow:"hidden"}}>{fD(t.start_date)}-{fD(t.end_date)}</span></div></div>
            {t.risk!=="On track"&&<div style={{width:8,height:8,borderRadius:"50%",background:RC[t.risk]?.c,flexShrink:0,animation:overdue?"pulse 1.5s infinite":"none"}}/>}
          </div>})}
      </div>})}</div>
    </div>}

    {/* BOARD with drag and drop */}
    {view==="board"&&<div className="ani-fade">
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:16}}>{STS.map(st=><div key={st}
        onDragOver={e=>e.preventDefault()} onDrop={()=>onDrop(st)}
        style={{background:"var(--bg2)",borderRadius:10,padding:12,minHeight:200,border:dragId?"2px dashed var(--border)":"2px solid transparent",transition:"border .2s"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>{tasks.filter(t=>t.status===st).map((t,idx)=>{const rc=RC[t.risk];const pc=PC[t.priority];const overdue=t.status!=="Done"&&t.end_date<today;
          return <div key={t.id} className="card-hover ani-slide" draggable onDragStart={()=>setDragId(t.id)} onDragEnd={()=>setDragId(null)}
            onClick={()=>setSel(t)} style={{background:"var(--card)",borderRadius:10,padding:12,border:"1px solid var(--border)",borderLeft:"4px solid "+(CL[t.dept]||"#94A3B8"),cursor:"grab",animationDelay:idx*40+"ms",boxShadow:overdue?"inset 0 0 0 1px #FCA5A5":"none"}}>
            <div style={{fontSize:13,fontWeight:600,marginBottom:6,color:"var(--fg)"}}>{t.name}{overdue&&<span style={{color:"#EF4444",fontSize:10,marginLeft:6}}>OVERDUE</span>}</div>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}><div style={{display:"flex",alignItems:"center",gap:4,fontSize:11,color:"var(--fg2)"}}><div style={{width:16,height:16,borderRadius:"50%",background:CL[t.dept],display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{color:"#fff",fontSize:7,fontWeight:700}}>{t.owner?.[0]}</span></div>{fD(t.start_date)}-{fD(t.end_date)}</div></div>
          </div>})}</div>
      </div>)}</div>
    </div>}

    {/* CALENDAR */}
    {view==="calendar"&&<CalendarView tasks={tasks} meetings={meetings} onSelect={setSel}/>}

    {/* RACI */}
    {view==="raci"&&<div className="ani-fade" style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{display:"flex",justifyContent:"space-between"}}><div style={{fontSize:14,fontWeight:800,color:"var(--fg)"}}>RACI Matrix</div><button onClick={()=>setAddModal("raci")} style={{background:"linear-gradient(135deg,#3B82F6,#6366F1)",color:"#fff",border:"none",padding:"6px 14px",borderRadius:8,fontWeight:600,fontSize:11,cursor:"pointer"}}>+ Add Entry</button></div>
    ...
    {/* KPIs */}
    {view==="kpi"&&<div className="ani-fade" style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{display:"flex",justifyContent:"space-between"}}><div style={{fontSize:14,fontWeight:800,color:"var(--fg)"}}>KPIs</div><button onClick={()=>setAddModal("kpi")} style={{background:"linear-gradient(135deg,#3B82F6,#6366F1)",color:"#fff",border:"none",padding:"6px 14px",borderRadius:8,fontWeight:600,fontSize:11,cursor:"pointer"}}>+ Add KPI</button></div>
      <KpiChart kpis={kpis}/>
      {Object.entries(kpiByDept).map(([dept,items])=><div key={dept} className="ani-slide"><DeptHdr dept={dept}/>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:8,padding:10,background:"var(--card)",border:"1px solid var(--border)",borderRadius:"0 0 8px 8px"}}>{items.map(k=><div key={k.id} className="card-hover" style={{borderLeft:"3px solid "+FC[k.flag],borderRadius:8,padding:12,background:"var(--bg2)",transition:"all .2s"}}>
    ...
    {view==="roles"&&<div className="ani-fade">
      ...
      <Tbl headers={["Role","Status","Trigger / Blocker","Target",""]} rows={roles.map(r=>[
        ...
        <button onClick={()=>deleteRole(r.id)} style={{background:"none",border:"none",color:"#DC2626",cursor:"pointer",fontSize:11}}>x</button>
      ])}/>
    </div>}

    {/* MEETINGS — dynamic with calendar */}
    {view==="meet"&&<div className="ani-fade">
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:12,flexWrap:"wrap",gap:8}}>
        <div style={{fontSize:14,fontWeight:800,color:"var(--fg)"}}>Meeting Cadence</div>
        <div style={{display:"flex",gap:8}}>
          <button onClick={()=>setAddModal("meeting")} style={{background:"linear-gradient(135deg,#3B82F6,#6366F1)",color:"#fff",border:"none",padding:"6px 14px",borderRadius:8,fontWeight:600,fontSize:11,cursor:"pointer"}}>+ Add Meeting</button>
          <a href="https://calendar.google.com/calendar/r/eventedit" target="_blank" rel="noopener" style={{fontSize:11,color:"#3B82F6",fontWeight:600,textDecoration:"none",background:"#EFF6FF",padding:"6px 14px",borderRadius:8,display:"flex",alignItems:"center"}}>+ Create in Google Calendar</a>
        </div>
      </div>
      <Tbl headers={["Type","Meeting","When","Duration","Owner","Attendees",""]} rows={meetings.map(m=>[
        ...
        <button onClick={()=>deleteMeeting(m.id)} style={{background:"none",border:"none",color:"#DC2626",cursor:"pointer",fontSize:11}}>x</button>
      ])}/>
    </div>}
